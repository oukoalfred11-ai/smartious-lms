...
const { mathToText, cleanForPaper } = require('./mathText');
...
function renderParts(doc, parts, depth, x, width, scheme) {
  parts.forEach((p, i) => {
    ...
    doc.fontSize(9.5).font('Helvetica').fillColor(INK)
       .text(esc(cleanForPaper(p.text)), x + 26, doc.y, { width: width - 26 - 34 })
    ...
    if (p.type === 'mcq' && Array.isArray(p.options) && p.options.length) {
      p.options.forEach((opt, oi) => {
        doc.fontSize(9).font('Helvetica').fillColor(INK)
           .text(`${ALPHA[oi].toUpperCase()}   ${esc(cleanForPaper(opt))}`, x + 46, doc.y, { width: width - 80 })
        doc.y += 2
      })
      doc.y += 4
    }
...
function renderQuestion(doc, q, number, scheme) {
  ...
  doc.fontSize(10).font('Helvetica').fillColor(INK)
     .text(esc(cleanForPaper(q.questionText)), x + 22, doc.y, { width: CW - 22 - 40 })
  doc.y += 6
  ...
  if (!hasParts && q.type === 'mcq' && Array.isArray(q.options) && q.options.length) {
    q.options.forEach((opt, oi) => {
      doc.fontSize(9.5).font('Helvetica').fillColor(INK)
         .text(`${ALPHA[oi].toUpperCase()}   ${esc(cleanForPaper(opt))}`, x + 40, doc.y, { width: CW - 80 })
      doc.y += 2
    })
    doc.y += 4
    if (!scheme) {
