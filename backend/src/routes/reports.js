...
<div style="font-size:8.5px;color:#777;line-height:1.7;font-style:italic;border-top:1px solid #ddd;padding-top:2mm">
  This report is an official document of Smartious Homeschool &amp; eSchool and is invalid without the official stamp. Queries: hellosmartious@gmail.com · +254 745 021 212 · Verification letters available per the published Fee Structure. Full mark breakdowns, examination scripts and learning plans are available in the Parent Portal.
</div>
...
async function notifyParentReportReady(report, student) {
  ...
  let sent = 0
  for (const email of emails) { try { await t.sendMail({ from:process.env.EMAIL_FROM||'Smartious <hellosmartious@gmail.com>', to:email, subject:`New report — ${report.studentName} · ${report.subject||'General'}`, html }); sent++ } catch(e) { console.error('[report notify]',email,e.message) } }
  return sent
}
