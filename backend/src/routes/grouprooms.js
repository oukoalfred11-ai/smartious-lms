  ...
  try {
    ...
    for (const r of rooms) {
      ...
      if (existing) {
        ...
        // Only write teacher if frontend sent one (avoid clobbering with empty)
        if (r.teacher && /^[a-f\d]{24}$/i.test(r.teacher)) {
          updateData.teacher = r.teacher;
        }
        ...
        if (r.teacher && /^[a-f\d]{24}$/i.test(r.teacher)) {
          createData.teacher = r.teacher;
        }
        await GroupRoom.create(createData);
        created++;
      }
    }
