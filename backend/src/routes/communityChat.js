...
const CAN_POST = ['student', 'teacher', 'parent', 'admin', 'dos', 'ops_manager'];
...
router.get('/messages', auth, async (req, res) => {
  try {
    ...
    const memberCount = await User.countDocuments({ role: { $in: ['student', 'teacher', 'parent', 'admin', 'dos', 'ops_manager'] }, isActive: { $ne: false } });
...
router.get('/members', auth, async (req, res) => {
  try {
    const rows = await User.find({ role: { $in: ['student', 'teacher', 'parent', 'admin', 'dos', 'ops_manager'] }, isActive: { $ne: false } })
