...
export default function CountryHub({
  ...
  return (
    <>
      ...
      {/* ─────────────────────────────────────────────────────────
          v2+ OPTIONAL SECTIONS — render only when the country data
          provides these fields. Countries without them render as before.
          
          Introduced with the Malaysia hub as a template for retrofitting
          other hubs. Each section can be added independently to any
          country's data file to enrich its hub incrementally.
          ───────────────────────────────────────────────────────── */}

      {/* ── Local statistics band ── */}
      {Array.isArray(country.localStatistics) && country.localStatistics.length > 0 && (
        <section className="sec" style={{background:V.bone,paddingTop:56,paddingBottom:56,borderTop:`1px solid ${V.bone3}`}}><div className="wrap">
          <div style={{maxWidth:1100,margin:'0 auto'}}>
            <div style={{textAlign:'center',marginBottom:36,maxWidth:780,marginLeft:'auto',marginRight:'auto'}}>
              <div className="eyebrow" style={{justifyContent:'center'}}>{country.name} in numbers</div>
              <h2 style={{fontFamily:"'DM Serif Display',Georgia,serif",fontSize:'2rem',fontWeight:400,color:V.ink,marginTop:8,marginBottom:14,lineHeight:1.2}}>
                Local context every {country.adjective || country.name} family should know
              </h2>
              <p style={{fontSize:14,color:V.sl,lineHeight:1.65}}>
                Sourced figures that inform the education decisions Smartious families make in {country.name}.
              </p>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))',gap:16}}>
              {country.localStatistics.map((s,i) => (
                <div key={i} style={{background:V.white,border:`1px solid ${V.bone3}`,borderRadius:10,padding:'20px 22px'}}>
                  <div style={{fontFamily:"'DM Serif Display',Georgia,serif",fontSize:26,color:V.cr,lineHeight:1.15,marginBottom:8,fontWeight:400}}>{s.number}</div>
                  <div style={{fontSize:13,color:V.ink,lineHeight:1.5,marginBottom:8,fontWeight:600}}>{s.label}</div>
                  {s.source && <div style={{fontSize:11,color:V.sl,lineHeight:1.5,fontStyle:'italic'}}>Source: {s.source}</div>}
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ── Comparison table ── */}
      {country.comparisonTable && Array.isArray(country.comparisonTable.rows) && country.comparisonTable.rows.length > 0 && (
        <section className="sec" style={{background:V.white,paddingTop:56,paddingBottom:56,borderTop:`1px solid ${V.bone3}`}}><div className="wrap">
          <div style={{maxWidth:1100,margin:'0 auto'}}>
            <div style={{textAlign:'center',marginBottom:36,maxWidth:780,marginLeft:'auto',marginRight:'auto'}}>
              ...
              {country.comparisonTable.intro && (
                <p style={{fontSize:14,color:V.sl,lineHeight:1.65}}>{country.comparisonTable.intro}</p>
              )}
            </div>
            <div style={{overflowX:'auto',border:`1px solid ${V.bone3}`,borderRadius:10}}>
              <table style={{width:'100%',minWidth:720,borderCollapse:'collapse',fontSize:13,color:V.ink}}>
                <thead>
                  <tr style={{background:V.ink,color:'#fff'}}>
                    {(country.comparisonTable.columns || ['Feature','Smartious','Alternative','Note']).map((col,i) => (
                      <th key={i} style={{padding:'14px 16px',textAlign:'left',fontWeight:700,fontSize:12,letterSpacing:'.02em',borderRight: i < 3 ? `1px solid rgba(255,255,255,.1)` : 'none'}}>{col}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {country.comparisonTable.rows.map((r,i) => (
                    <tr key={i} style={{background: i % 2 === 0 ? V.bone : V.white, borderTop:`1px solid ${V.bone3}`}}>
                      <td style={{padding:'12px 16px',fontWeight:700,lineHeight:1.5,verticalAlign:'top'}}>{r.feature}</td>
                      <td style={{padding:'12px 16px',color:V.cr,fontWeight:600,lineHeight:1.5,verticalAlign:'top'}}>{r.smartious}</td>
                      <td style={{padding:'12px 16px',color:V.sl,lineHeight:1.5,verticalAlign:'top'}}>{r.competitor}</td>
                      <td style={{padding:'12px 16px',fontSize:12,color:V.sl,lineHeight:1.5,verticalAlign:'top',fontStyle: r.note ? 'italic' : 'normal'}}>{r.note}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {/* ── Branch campus universities (Malaysia + potentially Qatar/UAE later) ── */}
      {Array.isArray(country.branchCampusUniversities) && country.branchCampusUniversities.length > 0 && (
        <section className="sec" style={{background:V.bone,paddingTop:56,paddingBottom:56,borderTop:`1px solid ${V.bone3}`}}><div className="wrap">
          <div style={{maxWidth:1100,margin:'0 auto'}}>
            <div style={{textAlign:'center',marginBottom:36,maxWidth:780,marginLeft:'auto',marginRight:'auto'}}>
              <div className="eyebrow" style={{justifyContent:'center'}}>University-in-country pathways</div>
              <h2 style={{fontFamily:"'DM Serif Display',Georgia,serif",fontSize:'2rem',fontWeight:400,color:V.ink,marginTop:8,marginBottom:14,lineHeight:1.2}}>
                Foreign university degrees awarded from within {country.name}
              </h2>
              <p style={{fontSize:14,color:V.sl,lineHeight:1.65}}>
                {country.name} hosts branch campuses of foreign universities that award the same degree as the home campus, at branch-campus fees. Cambridge A-Level and IB Diploma from Smartious feed directly into these programmes.
              </p>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:14}}>
              {country.branchCampusUniversities.map((u,i) => (
                <div key={i} style={{background:V.white,border:`1px solid ${V.bone3}`,borderRadius:10,padding:'20px 22px'}}>
                  <div style={{fontSize:14.5,color:V.ink,lineHeight:1.4,marginBottom:6,fontWeight:700}}>{u.name}</div>
                  <div style={{fontSize:12,color:V.gold3,fontWeight:700,letterSpacing:'.02em',marginBottom:12}}>{u.city}</div>
                  <div style={{fontSize:12.5,color:V.ink,lineHeight:1.6,marginBottom:8}}>
                    <strong style={{fontWeight:700}}>Accepts: </strong>{u.accepts}
                  </div>
                  {u.note && <div style={{fontSize:12,color:V.sl,lineHeight:1.55,fontStyle:'italic'}}>{u.note}</div>}
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ── Topical cluster links (SEO topical authority) ── */}
      {Array.isArray(country.topicalClusterLinks) && country.topicalClusterLinks.length > 0 && (
        <section className="sec" style={{background:V.white,paddingTop:56,paddingBottom:56,borderTop:`1px solid ${V.bone3}`}}><div className="wrap">
          <div style={{maxWidth:1100,margin:'0 auto'}}>
            <div style={{textAlign:'center',marginBottom:32,maxWidth:780,marginLeft:'auto',marginRight:'auto'}}>
              <div className="eyebrow" style={{justifyContent:'center'}}>Read more</div>
              <h2 style={{fontFamily:"'DM Serif Display',Georgia,serif",fontSize:'1.75rem',fontWeight:400,color:V.ink,marginTop:8,marginBottom:14,lineHeight:1.2}}>
                Related guides for {country.name} families
              </h2>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))',gap:12}}>
              {country.topicalClusterLinks.map((link,i) => (
                <a key={i} href={link.href} onClick={e => { e.preventDefault(); if (typeof nav === 'function') nav(link.href) }}
                  style={{textDecoration:'none',background:V.bone,border:`1px solid ${V.bone3}`,borderRadius:8,padding:'14px 16px',display:'block',transition:'border-color .15s'}}>
                  {link.category && <div style={{fontSize:10.5,color:V.gold3,fontWeight:700,letterSpacing:'.12em',textTransform:'uppercase',marginBottom:6}}>{link.category}</div>}
