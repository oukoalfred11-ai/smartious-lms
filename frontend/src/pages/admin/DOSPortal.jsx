...
const SMARTIOUS_THEME = `
  ...
  .sm-admin-theme .sec-tag {
    color: var(--crimson) !important; font-weight: 700;
  ...
  .sm-admin-theme .btn-p {
    ...
    box-shadow: 0 2px 8px rgba(125,16,37,0.18); font-weight: 700;
  }
  .sm-admin-theme .btn-p:hover {
    background: var(--crimson-deep) !important;
    border-color: var(--crimson-deep) !important;
  }
  .sm-admin-theme .card { transition: box-shadow 0.18s, border-color 0.18s; background: #fff; }
  .sm-admin-theme .card:hover { box-shadow: 0 8px 24px rgba(125,16,37,0.06); }
  .sm-admin-theme .kpi { transition: all 0.18s; background: #fff; }
  .sm-admin-theme .kpi:hover {
    border-color: var(--crimson) !important; transform: translateY(-2px);
    box-shadow: 0 12px 28px rgba(125,16,37,0.08);
  }
  .sm-admin-theme .nav-item-active,
  .sm-admin-theme [class*="nav"][class*="active"] {
    color: var(--crimson) !important; background: var(--b50) !important;
    border-left-color: var(--crimson) !important;
  }
  .sm-admin-theme .fi:focus, .sm-admin-theme .fsel:focus, .sm-admin-theme .fta:focus {
    border-color: var(--crimson) !important;
    box-shadow: 0 0 0 3px rgba(125,16,37,0.08) !important; outline: none !important;
  }
  .sm-admin-theme .tbl thead { background: var(--cream) !important; }
  .sm-admin-theme .tbl thead th {
    color: var(--crimson) !important; font-weight: 700 !important;
    letter-spacing: 0.06em; text-transform: uppercase; font-size: 11px !important;
  }
  @keyframes sm-pulse {
...
export default function DOSPortal() {
  ...
  useEffect(() => {
    api.get('/users/stats')
      .then(r => setUserStats(r.data.totalUsers || 0))
      .catch(() => {})
  }, [refreshKey])
  ...
  useEffect(() => {
    const fetch = () =>
      api.get('/allocations/pending-count')
        .then(r => setPendingAllocations(r.data.pendingCount || 0))
        .catch(() => {})
    fetch()
