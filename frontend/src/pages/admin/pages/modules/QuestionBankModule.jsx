...
function QuestionBankModule({ toast }) {
  ...
  const runDiagnose = async () => {
    setDiagBusy(true)
    try {
      const r = await api.get('/questions/homework-debug')
      const d = r.data?.data || {}
      setDiag({ checks: [], classes: d.classes || [] })
      toast?.ok?.(r.data?.message || 'Diagnosis complete.')
    } catch(e) {
      toast?.error?.(e?.response?.data?.message || 'Diagnose failed — is the backend deployed?')
      setDiag({ checks:[{ name:'Endpoint reachable', pass:false, detail:'GET /api/questions/homework-debug failed.' }], classes:[] })
    }
    setDiagBusy(false)
  }
