...
// MORNING only; after-school is impossible from our side. Peruvian
...
export const PERU_CITIES = [
  // ─────────────────────────────────────────────────────────────────
  {
    ...
    faqs: [
      ...
      { q: 'Eight hours — how does the timetable work?', a: 'Our classes land in the Peruvian morning, not the afternoon. For a student in turno tarde, whose school runs in the afternoon, that is the free half of the day. After-school is not possible from our side, and we say so upfront.' },
...
export const PERU_COUNTRY = {
  ...
  legalFramework: [
    ...
    { h: 'The timezone, and the school day that rescues it', p: 'We teach from Nairobi at UTC+3 and Peru runs PET at UTC-5 with no daylight saving, so the gap is a fixed eight hours — the same as Colombia and one hour better than Mexico. That means our teaching lands in the Peruvian morning and an after-school arrangement is simply not possible from our side; we would rather say so on this page than in a third conversation. What makes it workable is the Peruvian school day itself: schools commonly run turno mañana and turno tarde, so a student on the afternoon shift has mornings entirely free, which is exactly our window. Families whose child is in turno mañana should talk to us before enrolling so we can be realistic about which subjects and days are possible.' },
  ],
