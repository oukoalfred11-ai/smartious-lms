...
// the PANAMANIAN MORNING only. Panamanian schools commonly run
...
export const PANAMA_CITIES = [
  // ─────────────────────────────────────────────────────────────────
  {
    ...
    faqs: [
      ...
      { q: 'What are the class times?', a: 'We are eight hours ahead, so our classes land in the Panamanian morning. For a student in the afternoon turno that is the free half of the day. After-school is not possible from our side.' },
    ],
  },
...
export const PANAMA_COUNTRY = {
  ...
  legalFramework: [
    ...
    { h: 'The timezone, and the school day that makes it work', p: 'We teach from Nairobi at UTC+3 and Panama runs EST at UTC-5 with no daylight saving, so the gap is a fixed eight hours — the same as Ecuador, Colombia and Peru. Our teaching lands in the Panamanian morning, which means an after-school arrangement is not possible from our side, and we say so upfront. What makes it workable is the Panamanian school day: schools commonly run morning and afternoon turnos, so a student on the afternoon turno has mornings entirely free. Families whose child is on the morning turno should talk to us before enrolling so we can be realistic about which subjects and days are possible.' },
